...
📝 Next steps:
   ...
   2. Validate and reload Tor: docker exec tor-relay refresh
...
After updating the active torrc, validate and reload it without replacing the Tor process:
...
3.  **Reload**: Did `docker exec tor-relay refresh` succeed after editing the config?
